...
Note that the BAM file is not included so do not try to run this code.
<<eval=FALSE>>=
testbam <- "SAMPLE1.bam"
...
The first step is to extract all resions of interest (ROIs) that define the 
count windows from a BED file with the function \verb+getWindows+. 
<<>>=
